...
public class BatchConfig {
	...
	public FlatFileItemReader<User> reader() {
		return new FlatFileItemReaderBuilder<User>().name("userItemReader")
				...
				.names(new String[] { "firstName", "lastName" }).fieldSetMapper(new BeanWrapperFieldSetMapper<User>() {
					{
	...
	public JdbcBatchItemWriter<User> writer(DataSource dataSource) {
		return new JdbcBatchItemWriterBuilder<User>()
				...
				.sql("INSERT INTO user (first_name, last_name) VALUES (:firstName, :lastName)").dataSource(dataSource)
				.build();
	}
