...
public class SwaggerConfig {
	...
	public Docket api(ServletContext servletContext) {
		return new Docket(DocumentationType.SWAGGER_2).select()
				.apis(RequestHandlerSelectors.basePackage("com.folaukaveinga")).paths(PathSelectors.any()).build()
				.apiInfo(getApiInfo());
	}

	private ApiInfo getApiInfo() {
		Contact contact = new Contact("Folau Kaveinga", "http://lovemesomecoding.com", "folaukaveinga@gmail.com");
		return new ApiInfoBuilder().title("Jdbc MultiDBS API").description("Jdbc MultiDBS API").version("1.0.0")
